...
void print_struct_format(D3Struct* a, int b, int c) {
	char buf[200];
			...
			for (int j = 0; ; j++) {
				...
				*/
				snprintf(buf, sizeof(buf), "\tFIELD %s, %s, %d, %d, %d", name, *field, ((ProtocolFieldDescriptor*)field)->EncodedBits, ((ProtocolFieldDescriptor*)field)->Min, ((ProtocolFieldDescriptor*)field)->Max);
